% Almost the same as tntMatCreateProdMps, but here there is no 
% normalization of tensor.elems, since we are dealing with mixed states 
function wf = tntMatCreateProdMpsMixed(cfg,qnums)
...
for loop = 1:L
    if (~isempty(qnums))
        ...
        tensor.elems = cfg{loop}; % NO NORMALIZATION HERE SINCE CORRECT NORMALIZATION FOR MIXED STATES COMES FROM TRACE
